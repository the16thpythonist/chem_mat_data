"""
This experiment creates the processed graph dataset for **tmCAT** — a domain-specific
subset of ~21,600 mononuclear transition metal complexes associated with **catalysis**
...
et al. The CSD refcodes identifying catalytic complexes are extracted from the Zenodo
archive and joined with the tmQM 2024 release for SMILES and QM properties. This enables
domain-specific property prediction benchmarking on catalytic TMCs specifically.
...
**Usage**::
    ...
    python create_graph_datasets__tm_cat.py

Results are written to ``results/create_graph_datasets__tm_cat/debug/``.
"""
import os
import json
import gzip
...
#       Path within the Zenodo zip for the tmCAT application CSV.
APPLICATION_CSV_PATH: str = 'Data/Datasets/text_mined/tmCAT.csv'
...
DESCRIPTION: str = (
    'Graph dataset of ~21,600 mononuclear transition metal complexes associated with '
    'catalysis applications, curated from tmQM via NLP text mining (Kevlishvili et al., '
...
METADATA: dict = {
    'category': 'tmc',
    'min_version': '1.7.0',
    'tags': ['Molecules', 'TransitionMetals', 'TMC', 'QM', 'DFT', 'Catalysis'],
    'sources': [
        'https://doi.org/10.1039/D4FD00087K',
        'https://doi.org/10.5281/zenodo.11404217',
        'https://github.com/uiocompcat/tmQM',
    ],
    'verbose': 'tmCAT — Catalysis TMC Subset with QM Properties',
    'target_descriptions': {
...
def experiment(e: Experiment):
    ...
    e.log('starting tm_cat processing experiment...')
    e.log_parameters()
