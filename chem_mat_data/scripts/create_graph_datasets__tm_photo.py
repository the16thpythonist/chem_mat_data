"""
This experiment creates the processed graph dataset for **tmPHOTO** — a domain-specific
subset of ~4,600 mononuclear transition metal complexes associated with **photophysics**
...
et al. The CSD refcodes identifying photophysical complexes are extracted from the Zenodo
archive and joined with the tmQM 2024 release for SMILES and QM properties. This enables
domain-specific property prediction benchmarking on photophysical TMCs specifically.
...
**Usage**::
    ...
    python create_graph_datasets__tm_photo.py

Results are written to ``results/create_graph_datasets__tm_photo/debug/``.
"""
import os
import json
import gzip
...
#       Path within the Zenodo zip for the tmPHOTO application CSV.
APPLICATION_CSV_PATH: str = 'Data/Datasets/text_mined/tmPHOTO.csv'
...
DESCRIPTION: str = (
    'Graph dataset of ~4,600 mononuclear transition metal complexes associated with '
    'photophysics applications, curated from tmQM via NLP text mining (Kevlishvili et al., '
...
METADATA: dict = {
    'category': 'tmc',
    'min_version': '1.7.0',
    'tags': ['Molecules', 'TransitionMetals', 'TMC', 'QM', 'DFT', 'Photophysics'],
    'sources': [
        'https://doi.org/10.1039/D4FD00087K',
        'https://doi.org/10.5281/zenodo.11404217',
        'https://github.com/uiocompcat/tmQM',
    ],
    'verbose': 'tmPHOTO — Photophysics TMC Subset with QM Properties',
    'target_descriptions': {
...
def experiment(e: Experiment):
    ...
    e.log('starting tm_photo processing experiment...')
    e.log_parameters()
